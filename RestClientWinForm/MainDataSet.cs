...
namespace RestClientWinForm
{
    partial class MainDataSet
    {
        ...
        public async Task<string> KMTfill()
        {
            var dt = KMT;
            ...
            Stopwatch sw = new Stopwatch();
            sw.Start();
            using (var response = client.KMTfill(new QryProxy { Query = "abc" }))
            {
                while (await response.ResponseStream.MoveNext(token))
                {
                    //var proxy = response.ResponseStream.Current;
        ...
        public string KMTupdate()
        {
            StringBuilder sb = new StringBuilder();
            var dt = KMT;
            var request = new KMTproxy();
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (rs == "A" || rs == "M" || rs == "D")
                {
                    ...
                    var reply = client.KMTupdate(request);  // --------->
